...
import { variantOptions, PAGES, priceOf, AlbumVariant, PageCount } from "@/lib/products";
  ...
  return (
    <main style={{padding: 24, maxWidth: 900, margin: "0 auto"}}>
      ...
      <div style={{marginTop:16}}>
        ...
        <select value={String(pages)} onChange={e => setPages(Number(e.target.value) as PageCount)}>
          {PAGES.map(p => <option key={p} value={String(p)}>{p}</option>)}
        </select>
      </div>

      <div style={{marginTop:16}}>Cena: <b>{(price/100).toFixed(2)} €</b></div>

      <div style={{marginTop:16}}>
        <button onClick={()=>alert("Editor pridáme v ďalšom kroku")}>Pokračovať do editora</button>
      </div>
    </main>
  );
}